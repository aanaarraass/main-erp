		...
		async _clickProduct(event) { 
			...
			if (product.type == 'product'  && self.env.pos.config.display_stock_pos)
			{
				...
				$.each(result, function(key,value) {
					if(location[0] == value.location.id){
						value_qty = value.quantity
					}
				})

				if(self.env.pos.config.product_configurator == true) {
					await self._getAddProductOptions(product);
				}
				if(value_qty <= 0 || value_qty <= used_qty){
								...
								if(self.env.pos.loc_by_id && self.env.pos.loc_by_id[loc] && self.env.pos.loc_by_id[loc]['complete_name'])
								{
									...
									$.each(value, function( k, v ) {
										if(self.env.pos.loc_by_id[k] && self.env.pos.loc_by_id[k]['complete_name']){
											...
											loc_list.push([l_nm,v,self.env.pos.loc_by_id[k]['id']]);
											total_serial = total_serial + v
										}
									});
									if(qty > value[loc]){
				...
				$.each(orderlines, function( key, value ) {
						...
						$.each(prod_qty, function( k, v ) {
							if(self.env.pos.loc_by_id[k] && self.env.pos.loc_by_id[k]['complete_name']){
								...
								total_serial1 = total_serial1 + v;
							}
							
						});	
						if(value.stock_location_id == false && total_serial1 < value.quantity){
							if (value.product.type == "product"){
								let wrning = value.product.display_name + ': has only '+ total_serial1 +' Qty for location:"'+loc_name+'", So Please Update Quantity or select from other location.';
								let odrln = order.get_orderline(value.id);
								odrln.set_quantity(total_serial1);
								call_super = false;
								self.showPopup('PosOutOfStock', {
									'title': self.env._t('Out of Stock'),
									'warning':  self.env._t(wrning),
