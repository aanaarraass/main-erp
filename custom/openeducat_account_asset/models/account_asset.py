...
class AccountAssetCategory(models.Model):
    ...
    account_asset_id = fields.Many2one('account.account',
                                       ...
                                       domain=[('internal_type', '=', 'other'),
                                               ('deprecated', '=', False)],
                                       help="Account used to record the purchase of "
                                            "the asset at its original price.")
    account_depreciation_id = fields.Many2one('account.account',
                                              ...
                                              domain=[('internal_type', '=', 'other'),
                                                      ('deprecated', '=', False)],
                                              help="Account used in the depreciation "
    ...
    account_depreciation_expense_id = fields.Many2one('account.account',
                                                      ...
                                                      domain=[('internal_type',
                                                              ...
                                                              ('deprecated',
                                                               '=', False)],
                                                      help="Account used in the "
                                                           "periodical entries, "
                                                           "to record a part of the "
                                                           "asset as expense.")
    journal_id = fields.Many2one('account.journal', string='Journal', required=True)
    company_id = fields.Many2one('res.company', string='Company', required=True,
                                 default=lambda self: self.env['res.company'].
                                 _company_default_get('account.asset.category'))
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')],
                              string='Computation Method',
                              required=True, default='linear',
                              help="Choose the method to use to compute the amount"
    ...
    method_period = fields.Integer(string='Period Length', default=1,
                                   help="State here the time between 2 depreciations, "
                                        "in months", required=True)
    method_progress_factor = fields.Float('Degressive Factor', default=0.3)
    method_time = fields.Selection([('number', 'Number of Entries'),
                                    ('end', 'Ending Date')], string='Time Method',
                                   required=True, default='number',
                                   help="Choose the method to use to compute the dates "
                                        "and number of entries.\n"
                                        "  * Number of Entries: Fix the number "
                                        "of entries and the time between "
                                        "2 depreciations.\n"
                                        "  * Ending Date: Choose the time "
                                        "between 2 depreciations and the date the "
                                        "depreciations won't go beyond.")
    method_end = fields.Date('Ending date')
    prorata = fields.Boolean(string='Prorata Temporis',
                             help='Indicates that the first depreciation entry for '
                                  'this asset have to be done from the purchase date '
                                  'instead of the first of January')
    ...
    date_first_depreciation = fields.Selection([
        ...
        help='The way to compute the date of the first depreciation.\n'
             ...
             'dates will be based on the last day of the purchase month or '
             'the purchase year (depending on the periodicity of the depreciations).\n'
             '  * Based on purchase date: The depreciation dates will be based on '
             'the purchase date.')
...
class AccountAssetAsset(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'),
                             ...
                             help="When an asset is created, the status is 'Draft'.\n"
                                  "If the asset is confirmed, the status goes in "
                                  "'Running' and the depreciation lines can be posted"
                                  " in the accounting.\n"
                                  "You can manually close an asset when the "
                                  "depreciation is over. If the last line of"
                                  " depreciation is posted, the asset "
    ...
    method = fields.Selection([('linear', 'Linear'),
                              ...
                              help="Choose the method to use to compute "
                                   ...
                                   " * Linear: Calculated on basis of: Gross Value /"
    ...
    method_period = fields.Integer(string='Number of Months in a Period',
                                   ...
                                   states={'draft': [('readonly', False)]},
                                   help="The amount of time between "
                                        "two depreciations, in months")
    ...
    method_progress_factor = fields.Float(string='Degressive Factor',
                                          ...
                                          states={'draft': [('readonly', False)]})
    value_residual = fields.Float(compute='_compute_amount_residual', digits=0,
    ...
    method_time = fields.Selection([('number', 'Number of Entries'),
                                   ...
                                   states={'draft': [('readonly', False)]},
                                   help="Choose the method to use to compute "
                                        "the dates and number of entries.\n"
                                        "  * Number of Entries: Fix the number of "
                                        "entries and the time between "
                                        "2 depreciations.\n"
                                        "  * Ending Date: Choose the time between "
                                        "2 depreciations and the date the "
                                        "depreciations won't go beyond.")
    prorata = fields.Boolean(string='Prorata Temporis', readonly=True,
                             states={'draft': [('readonly', False)]},
                             help='Indicates that the first depreciation entry for '
                                  'this asset have to be done from the asset date '
                                  '(purchase date) instead of the first January / '
                                  'Start date of fiscal year')
    depreciation_line_ids = fields.One2many('account.asset.depreciation.line',
                                            'asset_id', string='Depreciation Lines',
                                            readonly=True,
                                            states={'draft': [('readonly', False)],
                                                    'open': [('readonly', False)]})
    salvage_value = fields.Float(string='Salvage Value', digits=0, readonly=True,
                                 states={'draft': [('readonly', False)]},
                                 help="It is the amount you plan to have "
                                      "that you cannot depreciate.")
    ...
    date_first_depreciation = fields.Selection([
        ...
        help='The way to compute the date of the first depreciation.\n'
             '  * Based on last day of purchase period: '
             'The depreciation dates will be based on the last day of the purchase '
             'month or the purchase year (depending on the periodicity '
             'of the depreciations).\n'
             '  * Based on purchase date: The depreciation dates will be '
             'based on the purchase date.\n')
    first_depreciation_manual_date = fields.Date(
        string='First Depreciation Date',
        readonly=True, states={'draft': [('readonly', False)]},
        help='Note that this date does not alter the computation of '
             'the first journal entry in case of prorata temporis assets.'
             'It simply changes its accounting date'
    )

    def unlink(self):
        for asset in self:
            if asset.state in ['open', 'close']:
                raise UserError(_('You cannot delete a document '
                                  'that is in %s state.') % (asset.state,))
            for depreciation_line in asset.depreciation_line_ids:
                if depreciation_line.move_id:
                    raise UserError(_('You cannot delete a document '
                                      'that contains posted entries.'))
    ...
    def compute_generated_entries(self, date, asset_type=None):
        # Entries generated : one by grouped category and
        # one by asset from ungrouped category
        created_move_ids = []
        type_domain = []
        if asset_type:
                ...
                if self.prorata:
                    ...
                    if sequence == 1:
                        ...
                        if self.method_period % 12 != 0:
                            ...
                            amount = (amount_to_depr / self.
                                      method_number) / month_days * days
                        else:
                            days = (self.company_id.compute_fiscalyear_dates(
                                date)['date_to'] - date).days + 1
                            amount = (amount_to_depr / self.
                                      method_number) / total_days * days
                ...
                if self.prorata:
                    if sequence == 1:
                        ...
                        if self.method_period % 12 != 0:
                            month_days = calendar.monthrange(date.year, date.month)[1]
                            days = month_days - date.day + 1
                            amount = (residual_amount * self.
                                      method_progress_factor) / month_days * days
                        else:
                            days = (self.company_id.compute_fiscalyear_dates(
                                date)['date_to'] - date).days + 1
                            amount = (residual_amount * self.
                                      method_progress_factor) / total_days * days
        return amount
        ...
        if self.value_residual != 0.0:
                ...
                if self.date_first_depreciation == 'last_day_period':
                    ...
                    if self.method_period == 12:
                        ...
                        if depreciation_date < self.date:
                            depreciation_date = depreciation_date + relativedelta(
                                years=1)
                elif self.first_depreciation_manual_date and (
                        self.first_depreciation_manual_date != self.date):
                    # depreciation_date set manually from the
                    # 'first_depreciation_manual_date' field
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                ...
                vals = {
                    ...
                    'depreciated_value': self.value - (self.
                                                       salvage_value + residual_amount),
                ...
                # datetime doesn't take into account that the
                # number of days is not the same for each month
                if not (self.prorata and self.method_period % 12 != 0) and (
                        self.date_first_depreciation == 'last_day_period'):
                    max_day_in_month = calendar.monthrange(
                        depreciation_date.year, depreciation_date.month)[1]
    ...
    def _get_disposal_moves(self):
        move_ids = []
        for asset in self:
            unposted_depreciation_line_ids = asset.depreciation_line_ids. \
                filtered(lambda x: not x.move_check)
            if unposted_depreciation_line_ids:
                ...
                changes, tracking_value_ids = asset._mail_track(tracked_fields,
                                                                old_values)
                if changes:
                    asset.message_post(subject=_('Asset sold or disposed. Accounting '
                                                 'entry awaiting for validation.'),
                                       tracking_value_ids=tracking_value_ids)
    ...
    def _check_prorata(self):
        if self.prorata and self.method_time != 'number':
            raise ValidationError(
                _('Prorata temporis can be applied only for the '
                  '"number of depreciations" time method.'))
    ...
    def create(self, vals):
        asset = super(AccountAssetAsset, self.with_context(mail_create_nolog=True)). \
            create(vals)
...
class AccountAssetDepreciationLine(models.Model):
    ...
    move_check = fields.Boolean(compute='_compute_get_move_check',
                                string='Linked', store=True)
    move_posted_check = fields.Boolean(compute='_compute_get_move_posted_check',
    ...
    def _compute_get_move_posted_check(self):
        for line in self:
            line.move_posted_check = True if \
                line.move_id and line.move_id.state == 'posted' else False

    def create_move(self, post_move=True):
        created_moves = self.env['account.move']
        for line in self:
            if line.move_id:
                raise UserError(_('This depreciation is already linked to a journal'
                                  ' entry. Please post or delete it.'))
        ...
        if post_move and created_moves:
            created_moves.filtered(
                lambda m: any(m.asset_depreciation_ids.
                              mapped('asset_id.category_id.open_asset'))).action_post()
    ...
    def _prepare_move(self, line):
        ...
        depreciation_date = self.env.context.get(
            'depreciation_date') or (line.depreciation_date
        ...
        move_line_1 = {
            ...
            'analytic_account_id': account_analytic_id.id if
            category_id.type == 'sale' else False,
            'analytic_tag_ids': [(6, 0, analytic_tag_ids.ids)] if
            category_id.type == 'sale' else False,
            ...
            (1.0 * line.amount) or 0.0,
        }
        move_line_2 = {
            'name': asset_name,
            'account_id': category_id.account_depreciation_expense_id.id,
            'credit': 0.0 if
            float_compare(amount, 0.0, precision_digits=prec) > 0 else -amount,
            'debit': amount if
            float_compare(amount, 0.0, precision_digits=prec) > 0 else 0.0,
            'partner_id': line.asset_id.partner_id.id,
            'analytic_account_id': account_analytic_id.id if
    ...
    def _prepare_move_grouped(self):
        ...
        # we can suppose that all lines have the same category
        category_id = asset_id.category_id
        account_analytic_id = asset_id.account_analytic_id
        analytic_tag_ids = asset_id.analytic_tag_ids
        depreciation_date = self.env.context.get('depreciation_date') or fields. \
            Date.context_today(self)
        amount = 0.0
        for line in self:
