...
class AccountInvoiceLine(models.Model):
    ...
    asset_start_date = fields.Date(string='Asset Start Date',
                                   compute='_compute_asset_date',
                                   readonly=True, store=True)
    asset_end_date = fields.Date(string='Asset End Date', compute='_compute_asset_date',
                                 readonly=True, store=True)
    asset_mrr = fields.Float(string='Monthly Recurring Revenue',
                             compute='_compute_asset_date', readonly=True,
                             digits="Account", store=True)

    @api.depends('asset_category_id', 'move_id.invoice_date')
    def _compute_asset_date(self):
        for rec in self:
            rec.asset_mrr = 0
        ...
        if self.product_id:
            for move in self.move_id:
                if move.move_type == 'out_invoice':
                    self.asset_category_id = self.product_id.product_tmpl_id.\
    ...
    def get_invoice_line_account(self, invoice_line_type, product, fpos, company):
        return product.asset_category_id.account_asset_id or super(
            AccountInvoiceLine, self).get_invoice_line_account(
            invoice_line_type, product, fpos, company)
