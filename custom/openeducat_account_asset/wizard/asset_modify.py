...
class AssetModify(models.TransientModel):
    ...
    asset_method_time = fields.Char(compute='_compute_get_asset_method_time',
                                    string='Asset Method Time', readonly=True)

    def _compute_get_asset_method_time(self):
    ...
    #         and doc.xpath("//field[@name='method_end']"):
    #             node = doc.xpath("//field[@name='method_end']")[0]
    #             node.set('invisible', '1')
    #             setup_modifiers(node, result['fields']['method_end'])
    #             elif asset.method_time == 'end' and doc.
    #             xpath("//field[@name='method_number']"):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('active_id'):
            active_asset = self.env['account.asset.asset']. \
                browse(self.env.context.get('active_id'))
