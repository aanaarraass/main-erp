...
class OnboardingController(http.Controller):
    ...
    @http.route('/openeducat_assignment_enterprise/'
                'openeducat_assignment_onboarding_panel',
                auth='user', type='json')
    def openeducat_assignment_onboarding_panel(self):
        """ Returns the `banner` for the sale onboarding panel.
            It can be empty if the user has closed it or if he doesn't have
            the permission to see it. """
        company = request.env.user.company_id
        if not request.env.user._is_admin() or \
           company.openeducat_assignment_onboard_panel == 'closed':
            return {}

        return {
            'html': request.env.ref(
                'openeducat_assignment_enterprise.'
                'openeducat_assignment_onboarding_panel')._render({
                    'company': company,
                    'state': company.update_assignment_onboarding_state()
                })
        }
