...
class OnboardingController(http.Controller):
    ...
    @http.route('/openeducat_attendance_enterprise/'
                'openeducat_attendance_onboarding_panel',
                auth='user', type='json')
    def openeducat_attendance_onboarding_panel(self):
        ...
        if not request.env.user._is_admin() or \
           company.openeducat_attendance_onboard_panel == 'closed':
            return {}

        return {
            'html': request.env.ref(
                'openeducat_attendance_enterprise.'
                'openeducat_attendance_onboarding_panel')._render({
                    'company': company,
                    'state': company.update_attendance_onboarding_state()
                })
        }
