...
class OnboardingController(http.Controller):
    ...
    @http.route('/openeducat_campus_enterprise/'
                'openeducat_campus_enterprise_onboarding_panel',
                auth='user', type='json')
    def openeducat_campus_enterprise_onboarding_panel(self):
        ...
        if not request.env.user._is_admin() or \
           company.campus_enterprise_onboard_panel == 'closed':
            return {}

        return {
            'html': request.env.ref(
                'openeducat_campus_enterprise.'
                'openeducat_campus_enterprise_onboarding_panel')._render({
                    'company': company,
                    'state': company.update_campus_onboarding_state()
                })
        }
