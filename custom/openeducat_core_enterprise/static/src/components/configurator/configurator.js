...
class FeaturesSelectionScreen extends Component {
    ...
    async buildWebsite() {
        ...
        if (!themes.length) {
            this.env.router.navigate({to: 'CONFIGURATOR_FEATURES_SELECTION_OPENEDUCAT_SCREEN'});
        } else {
...
async function makeEnvironment() {
    ...
    store.on("update", null, () => {
        const newState = {
            ...
            selectedLabel: store.state.selectedLabel,
    ...
    const loaderTemplate = await owl.utils.loadFile('/openeducat_core_enterprise/static/src/xml/theme_preview.xml');
    const configuratorTemplates = await owl.utils.loadFile('/openeducat_core_enterprise/static/src/components/configurator/configurator.xml');
