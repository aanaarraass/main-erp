...
class DashboardProElement(models.Model):
    ...
    def create(self, values):
        ...
        if len(self) > 0:
            if values.get("type_of_element") == "to_do":
                to_do_list_line = []
                ...
                values["to_do_list_line"] = [(6, 0, to_do_list_line)]

        if values.get("list_fields_data", False) and values.get(
            "list_data_grouping", False
        ):
            many2many_field_ordering = {
                "list_fields_data": values["list_fields_data"][0][2],
                "list_data_grouping": values["list_data_grouping"][0][2],
            }
    ...
    def get_data_main_func(self, element_id, domain, sequence):
        ...
        if action_line.chart_type == "list_view":
            ...
            count = 0
            for pro in record.action_lines:
                count += 1
