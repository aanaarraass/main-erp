odoo.define('openeducat_lms_website.upload_material', function (require) {
'use strict';
...
var LmsUploadDialog = Dialog.extend({
    template: 'openeducat.upload.material',
//    xmlDependencies: ['/openeducat_lms_website/static/src/xml/material_upload.xml'],
    ...
    _bindSelect2Dropdown: function () {
        ...
        this.$('#category_id').select2(this._select2Wrapper(_t('Section'), false,
            function () {
                return self._rpc({
                    route: '/course/section/search_read',
                    params: {
                        fields: ['name'],
                        domain: [['course_id', '=', self.channelID]],
                    }
                });
            })
        );
    ...
    _fetchUrlPreview: function (url) {
        return this._rpc({
            route: '/course/prepare_preview/',
            params: {
                'url': url,
                'course_id': this.channelID,
            },
        });
    },
    ...
    */
    ...
    _setup: function () {
        this.slide_type_data = {
            presentation: {
                ...
                template: 'openeducat_lms_website.upload.modal.presentation',
            },
            webpage: {
                icon: 'fa-file-text',
                label: _t('Web Page'),
                template: 'openeducat_lms_website.upload.modal.webpage',
            },
            video: {
                icon: 'fa-video-camera',
                label: _t('Video'),
                template: 'openeducat_lms_website.upload.modal.video',
            },
            quiz: {
                icon: 'fa-question-circle',
                label: _t('Quiz'),
                template: 'openeducat_lms_website.upload.modal.quiz'
            }
        };
    },
    /**
    ...
    _onChangeType: function () {
        ...
        if (currentType === '_select') {
            tmpl = 'openeducat_lms_website.upload.material.select';
        } else if (currentType === '_upload') {
            tmpl = 'openeducat_lms_website.upload.material.uploading';
        } else if (currentType === '_import') {
            tmpl = 'openeducat_lms_website.upload.modal.import';
        } else {
    ...
    _onChangeSlideUpload: function (ev) {
        ...
        if (!file) {
            this.$('#slide-image').attr('src', '/openeducat_lms_website/static/src/img/document.png');
        ...
        this._fetchUrlPreview(url).then(function (data) {
            ...
            if (data.error) {
                ...
                if (data.total_time) {
                    ...
                    self._formSetFieldValue('duration', Math.round(data.total_time * 60));
                }
...
publicWidget.registry.websiteSlidesUploadLms = publicWidget.Widget.extend({
    selector: '.o_wslides_js_slide_upload_lms',
    xmlDependencies: ['/openeducat_lms_website/static/src/xml/material_upload.xml'],
    events: {
    ...
    _openDialog: function ($element) {
        ...
        return new LmsUploadDialog(this, data).open();
    },
...
return {
    LmsUploadDialog: LmsUploadDialog,
    websiteSlidesUploadLms: publicWidget.registry.websiteSlidesUploadLms
};
