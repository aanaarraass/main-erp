odoo.define('openeducat_meeting_enterprise.portal_meeting', function (require) {
    "use strict";
    ...
    const PortalMeetingWidget = publicWidget.Widget.extend({
        ...
        InitKendo: async function(){
            ...
            var stud_id = $(".stud_id_meeting_parent").attr('id');
            var timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
            ajax.jsonRpc("/get-meeting/data",'call',{
                stud_id:stud_id,
                current_timezone: timezone
            }).then(function(data){

                var kendoData = new kendo.data.SchedulerDataSource({data : data});
                $('#meeting_portal_kendo').kendoScheduler({
                    date   : new Date(date),
                    editable: {
                        confirmation: false,
                        create: false,
