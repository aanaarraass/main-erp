...
class OnboardingController(http.Controller):
    ...
    @http.route(
        '/openeducat_quiz/openeducat_quiz_onboarding_panel',
        auth='user', type='json')
    def openeducat_quiz_onboarding_panel(self):
        ...
        if not request.env.user._is_admin() or \
           company.quiz_enterprise_onboard_panel == 'closed':
            return {}

        return {
            'html': request.env.ref(
                'openeducat_quiz.openeducat_quiz_onboarding_panel')._render({
                    'company': company,
                    'state': company.update_quiz_onboarding_state()
                })
        }
