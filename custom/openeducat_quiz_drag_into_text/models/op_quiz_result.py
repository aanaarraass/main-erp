...
class QuizResultDragIntoText(models.Model):
    ...
    @api.depends('line_ids', 'line_ids.line_ids.name',
                 'line_ids.name', 'line_ids.line_ids.name')
    def _compute_get_result(self):
        res = super(QuizResultDragIntoText, self)._compute_get_result()
        for obj in self:
        ...
        for line in obj.line_ids:
            ...
            if not line.name:
                total_not_attempt += 1
                continue
            if line.que_type == 'drag_into_text':
                if line.given_answer == line.answer:
                    total_correct += 1
