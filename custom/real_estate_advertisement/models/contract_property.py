...
class PropertyContract(models.Model):
    ...
    # discount fields added by waqasalirandhawa@gmail.com
    discount_type = fields.Selection([('percent', 'Percent%'),('fixed', 'Fixed Amount')],string='Discount Type' ,defualt='fixed')
    discount_value = fields.Monetary('Add Percentage')
    ...
    @api.onchange('discount_value', 'discount_amount')
    def _get_discount_amount(self):
        for rec in self:
            if rec.discount_type == 'percent':
                rec.discount_amount = rec.property_selling_price * rec.discount_value / 100
    ...
    def create_invoice(self):
        return {
            ...
            "target": "new",
            "context": {
                "default_total_amount": self.contract_total_amount_with_tax + self.discount_amount,
    ...
    def _onchange_responsible_person_id(self):
        print(' def _onchange_responsible_person_id(self):=======')
    ...
    def _onchange_property_id(self):
        print('  def _onchange_property_id(self)::=======')
        # if self.property_id:
    ...
    def _compute_paid_installment(self):
        for contract in self:
            ...
            if contract.discount_amount:
                total_emi = total_emi - contract.discount_amount
                total_balance = total_balance - contract.discount_amount
    ...
    def create_installment(self):
        ...
        tax_amount = 0
        amount = 0
        if self.payment_paid != 'all':
            if self.down_payment_amount < 0.0:
                raise UserError('Down payment cannot be less than 0.0!')
        if self.payment_paid == "rental_installment":
            ...
            amount = self.property_selling_price -self.discount_amount
            ...
            if self.tax_ids:
                ...
                print(tax_amount_dict)
                tax_amount = tax_amount_dict['total_included'] - tax_amount_dict['total_excluded']
            total_amount = amount + tax_amount + self.discount_amount
        ...
        return {
            ...
            "context": {
                ...
                "default_config_installment_id": self.property_id.installment_scheme_ids[0].id,
                "default_untaxed_total_amount": amount,
                "default_down_payment": self.down_payment_amount,
