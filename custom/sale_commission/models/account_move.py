...
class AccountMove(models.Model):
    ...
    def _get_settlement(self, agent, company, date_from):
    ...
    def _prepare_settlement_vals(self, agent, company, date_from):
        return {
            "agent_id": agent.id,
            "company_id": company.id,
            "date_from": date_from,
    ...
    def action_settle(self):
        ...
        for invoice_lines in self.invoice_line_ids:
            for all_agent in invoice_lines.agent_ids:
                agents += all_agent
        if self.invoice_date:
            date_from = self.invoice_date
        else:
            date_from = fields.Date.today()
        for agent in agents:
            settlement = self._get_settlement(
                agent.agent_id, agent.company_id, date_from
            )
            if not settlement:
