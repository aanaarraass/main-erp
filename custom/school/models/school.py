...
class SchoolStandard(models.Model):
    ...
    school_class_section = fields.Selection([
        ('A', 'A'),
        ('B', 'B'),
        ('C', 'C'),
    ], string='Section', default='A')
    ...
    def onchange_combine(self):
        ...
        # division_id.name
        self.name = (
            str(self.standard_id.name) + "-" + str(self.school_class_section)
        )
    ...
    def check_standard_unique(self):
        ...
        if self.env["school.standard"].search(
            [
                ...
                ("school_class_section", "=", self.school_class_section),
                # ("division_id", "=", self.division_id.id),
                ("school_id", "=", self.school_id.id),
                ("id", "not in", self.ids),
            ]
        ):
            raise ValidationError(_("Class and Section should be unique!"))
    ...
    def name_get(self):
        ...
        return [
            (rec.id, rec.standard_id.name + "[" + rec.school_class_section + "]")
            for rec in self
        ]
...
class SchoolSchool(models.Model):
    ...
    def create(self, vals):
        ...
        school_obj = self.env['school.school'].search([])
        if len(school_obj) > 1:
            raise odoo.exceptions.ValidationError('Only One school can be created')
