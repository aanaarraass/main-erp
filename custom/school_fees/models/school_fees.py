...
class StudentFeesRegister(models.Model):
    ...
    _sql_constraints = [
        (
            "unique_year_month",
            "UNIQUE(standard_id, academic_year, academic_month)",
            "Fee for this month is already registered",
        )
    ]
    ...
    academic_month = fields.Many2one(comodel_name='academic.month', string="Academic Month")

    @api.onchange('academic_year')
    def get_related_months(self):
        for rec in self:
            return {'domain': {'academic_month': [('year_id', '=', rec.academic_year.id)]}}
    ...
    def fees_register_confirm(self):
        ...
        for rec in self:
            if not rec.journal_id and not self._context.get('is_demo_record'):
            ...
            for stu in stud_obj.search(
                ...
                if slip_obj.search(
                    ...
                    raise ValidationError(
                        _(
                            """There exists a Fees record for: %s for same date.!
"""
                        )
                        % stu.name
                    )
...
class StudentPayslipLine(models.Model):
    ...
    amount = fields.Float("Fee Amount", digits=(16, 2), help="Fee amount")
    line_ids = fields.One2many(
    ...
    currency_id = fields.Many2one("res.currency", "Currency", help="Select currency",
                                  default=lambda self: self.env.user.company_id.currency_id)
    currency_symbol = fields.Char(related="currency_id.symbol", string="Symbol", help="Currency Symbol")
