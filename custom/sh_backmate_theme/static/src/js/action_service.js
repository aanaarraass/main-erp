...
//	let shcontrollerStack = []
    ...
    // function _getCurrentshController() {
    //     const stack = controllerStack;
    //     return stack.length ? stack[stack.length - 1] : null;
    // }
    // Softhealer code end
    ...
    async function _updateUI(controller, options = {}) {
        ...
        // if(options['click_tab'] == 1){
        //     var nextStack = shcontrollerStack.slice(0, index).concat(controllerArray);
        //     controller.config.breadcrumbs = _getBreadcrumbs(nextStack.slice(-2,-1));
        //     console.log("controller.config.breadcrumbs",controller.config.breadcrumbs)
        //     if (controller.Component.isLegacy) {
        //         controller.props.breadcrumbs = controller.config.breadcrumbs;
        //     }
        // }else{
            //softhealer code start
            var nextStack = controllerStack.slice(0, index).concat(controllerArray);
            if(enable_multi_tab){
                if('index' in options){
                    if('click_tab' in options){
                        controller.config.breadcrumbs = _getBreadcrumbs(nextStack.slice(-1, -1));
                    }else{
                        controller.config.breadcrumbs = _getBreadcrumbs(nextStack.slice(-2, -1));
                    }
            ...
            mounted() {
                if (action.target === "new") {
                    ...
                    controllerStack = nextStack; // the controller is mounted, commit the new stack
					 // Softhealer Code start
                    // shcontrollerStack = nextStack;
                   
				//	$.merge( shcontrollerStack, nextStack )
                    // Softhealer Code End
        ...
        // Softhealer Code Start
        // if(options['click_tab'] == 1){
        //     const currentController = _getCurrentshController();
        // }
         // Softhealer Code End
            ...
            $sh_nav_item_btn.on('click', function(sh_e) {
               // e.preventDefault();
                ...
                if(!$(this).hasClass('sh_active')){
                    ...
                    restore_tab(sh_e.currentTarget.dataset.controller);
                }
    ...
    async function _executeClientAction(action, options) {
        ...
        if (clientAction.prototype instanceof Component) {
            ...
            // Softhealer code start
            if(enable_multi_tab){
                console.log("localStorage.getItem",localStorage.getItem("sh_add_tab"))
                if(localStorage.getItem("sh_add_tab") == 1 ||  $("#sh_owl_items div").length == 0 || action.xml_id =="crm.crm_lead_action_pipeline" || action.xml_id =="crm.crm_lead_action_forecast" ){
                    shAddTabElement(controller, action, options)
    ...
    async function _executeServerAction(action, options) {
        console.log("Execute Server Action")
    ...
    // Softhealer Code start
	 ...
	 async function restore_tab(jsId) {
        let index;
        if (!jsId) {
        ...
        await clearUncommittedChanges(env);
        return _updateUI(controller, {index, click_tab:1 });
    }
    // Softhealer Code end
